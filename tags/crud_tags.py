...
def load_data():
    try:
        ...
        cursor = db.cursor()
        query = "SELECT id, name, user_id FROM tags ORDER BY id ASC"
...
def simpan_data(id, name, user_id):
    try:
        db = create_connection()
        cursor = db.cursor()
        query = "INSERT INTO tags (id, name, user_id) VALUES (%s, %s, %s)"
        cursor.execute(query, (id, name, user_id))
        db.commit()
        db.close()
        return True, "Tags berhasil disimpan."
...
def ubah_data(old_id, new_id, name, user_id):
    try:
        db = create_connection()
        cursor = db.cursor()
        query = "UPDATE tags SET id=%s, name=%s, user_id=%s WHERE id=%s"
        cursor.execute(query, (new_id, name, user_id, old_id))
        db.commit()
        db.close()
        return True, "Tags berhasil diubah."
    except Exception as e:
...
def hapus_data(id):
    try:
        ...
        db.commit()
        db.close()
        return True, "Tags berhasil dihapus."
    except Exception as e:
