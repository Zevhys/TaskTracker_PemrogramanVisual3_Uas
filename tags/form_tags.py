...
from tags import crud_tags as crud
import utils
...
class FormTags(QWidget):
    def __init__(self):
        ...
        ui_path = os.path.join(current_dir, "tags.ui")
    ...
    def tampilkan_data(self):
        ...
        if success:
            ...
            for row, tags in enumerate(data):
                self.tableWidget.setItem(row, 0, QTableWidgetItem(str(tags["id"])))
                self.tableWidget.setItem(row, 1, QTableWidgetItem(tags["name"]))
                self.tableWidget.setItem(row, 2, QTableWidgetItem(str(tags["user_id"])))
    ...
    def handle_simpan(self):
        ...
        success, msg = crud.simpan_data(id, name, userId)
        if success:
    ...
    def handle_ubah(self):
        ...
        success, msg = crud.ubah_data(self.old_id, new_id, name, userId)
        if success:
            QMessageBox.information(self, "Sukses", msg)
            self.tampilkan_data()
            self.clear_inputs()
    ...
    def handle_cetak(self):
        utils.export_table_to_pdf(self.tableWidget, "Data Tags", self)
